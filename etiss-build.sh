...
for arg in "$@"; do
    if [ "$VLEN_NEXT" = "$TRUE" ]; then
        ...
        >&2 echo "Args: [--ml, --debug, --trace, --float, --llvm, --vlen <VLEN>, --arch <ARCH>, (-c, --clean)]"
        exit 1
    fi
done

ZVL_STRING=zvl${VLEN}b
if [ "$ARCH" = "$SCALAR_ARCH" ]; then
    ZVL_STRING=""
fi
...
VERILATOR_SOURCE_FLAGS="-DVERILATOR=On -DMIN_VLEN=$VLEN -DTRACE=$SW_TRACE"

VECTOR_DIR=build/$ARCH/$ZVL_STRING
INSTALL_PATH_ETISS=$WS_PATH/gen_perfsim/target_sw/examples/Vicuna/custom/$ARCH/$ZVL_STRING
VERILATOR_DIR=$WS_PATH/vicuna2_tinyml_benchmarking/build_from_other/$ARCH/$ZVL_STRING
...
cmake --build $VECTOR_DIR -j$(nproc)
cmake --build $VERILATOR_DIR -j$(nproc)
cmake --install $VECTOR_DIR --prefix=$INSTALL_PATH_ETISS

# Copy prog files
cp $VERILATOR_DIR/target_code/vector/prog_* $VERILATOR_DIR/prog
if [ "$ML_FLAG" = "-DML=On" ]; then
    cp $VERILATOR_DIR/target_code/ml_bench/*/*/prog_* $VERILATOR_DIR/prog
fi
...
echo -e "${MAGENTA}Using $GCC_PATH_VECTOR for vector, $ZVL_STRING${NC}"
