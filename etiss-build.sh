...
if [ "$#" -ge 1 ] && [ $1 = "debug" ]; then
    ...
    # Scalar
    # cmake -S . -B $SCALAR_DIR -DCMAKE_TOOLCHAIN_FILE=$SCALAR_TOOLCHAIN -DRISCV_TOOLCHAIN_PREFIX=$GCC_PATH_SCALAR -DCMAKE_BUILD_TYPE=Debug -DCMAKE_C_FLAGS_DEBUG="-g -Og" -DCMAKE_INSTALL_PREFIX=$INSTALL_PATH_ETISS
    # Vector
    ...
    cmake -S . -B $VERILATOR_DIR -DRISCV_ARCH=$VECTOR_ARCH -DRISCV_ABI=$VECTOR_ABI -DRISCV_TOOLCHAIN_PREFIX=$GCC_PATH_VECTOR -DCMAKE_TOOLCHAIN_FILE=$VECTOR_TOOLCHAIN $VERILATOR_SOURCE_FLAGS -DCMAKE_BUILD_TYPE=Debug -DCMAKE_C_FLAGS_DEBUG="-g -Og" -DCMAKE_INSTALL_PREFIX=$INSTALL_PATH_ETISS -DVECTOR=On -DVERILATOR=On
