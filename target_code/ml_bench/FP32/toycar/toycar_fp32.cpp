...
int run_test() {
  ...
  for (size_t i = 0; i < toycar_fp32_data_sample_cnt; i++) {
    memcpy(interpreter.input(0)->data.f, (float *)toycar_fp32_input_data[i],
           toycar_fp32_input_data_len[i] * 4); // Bytes to copy is 4*len
    ...
    for (size_t j = 0; j < toycar_fp32_input_data_len[i]; j++) {
      sum += pow((float)toycar_fp32_input_data[i][j] -
                     interpreter.output(0)->data.f[j],
                 2);
    }
    ...
    float diff =
        sum -
        toycar_fp32_output_data_ref[i]; // TODO:abs() makes negative error 0.00?
    ...
    if (diff > 0.001f | diff < -0.001f) {
      ...
      uart_printf("ERROR: at #%d, sum %f ref %f diff %f \n", i, sum,
                  toycar_fp32_output_data_ref[i], diff);
#endif
      return -1;
    } else {
#if defined(PRINT_OUTPUTS)
