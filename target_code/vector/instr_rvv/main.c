...
extern BenchFunc bench_mf8, bench_mf4, bench_mf2, bench_m1, bench_m2, bench_m4,
    bench_m8;
static BenchFunc *benches[] = {&bench_mf8, &bench_mf4, &bench_mf2, &bench_m1,
                               &bench_m2,  &bench_m4,  &bench_m8};
...
static int run_all_types(char const *name, ux bIdx, ux vl, int ta, int ma) {
  ux arr[RUNS];
  ...
  for (ux sew = 0; sew < (MAX_SEW_RAW + 1); ++sew)
    for (ux lmul_idx = 0; lmul_idx < 7; ++lmul_idx) {
      ...
      if (bench_ptr == NULL) { // NULL reached!
        return -1;
      }
      ...
      // mlonmcu_printf("<td>%d.%02d</td>", ((int)(val * 100) / 100), ((int)(val
      // * 100) % 100));
#else
        // mlonmcu_printf("<td>%.2f</td>", (double)(sum
        // * 1.0f/(UNROLL*LOOP*count*8)));
#endif
      continue;
    skip:
      // mlonmcu_printf("<td></td>");
